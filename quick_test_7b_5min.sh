...
# 创建输出目录
OUTPUT_DIR="$WORK_DIR/evaluation/evaluation_outputs/quick_7b_5min_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUTPUT_DIR"
...
export EVAL_NOTE="quick-7b-5min-test"
...
if [ $? -eq 0 ]; then
    ...
    if [ -n "$OUTPUT_JSONL" ]; then
        ...
        echo "   2. 对比14B模型: ./quick_test_5min.sh"
        echo "   3. 运行更长时间测试: ./quick_test_7b_10min.sh"
    fi
else
    echo ""
    echo "❌ 测试失败,请检查错误信息"
